...
(async () => {
 ...
 await card.locator('label:has-text("Time Format")').locator('xpath=following-sibling::select[1]').selectOption('custom');
 const labels = await card.locator('label').allTextContents();
 const inputs = await card.locator('input').evaluateAll((els)=>els.map(el=>({type:el.type,placeholder:el.placeholder,value:el.value,className:el.className})));
 console.log(JSON.stringify({labels,inputs},null,2));
 await browser.close();
})();
